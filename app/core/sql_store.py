...
def _json_default(value):
    ...
    if isinstance(value, (date, datetime)): return {"__date__": value.isoformat()}
...
class SqlRepository(Generic[T]):
    def __init__(self, engine: Engine, model_type: type[T], id_field: str):
        self.engine, self.model_type, self.id_field = engine, model_type, id_field
    ...
    def save(self, record: T) -> T:
        ...
        record_id, business_id, now = str(getattr(record, self.id_field)), str(getattr(record, "business_id")), datetime.utcnow()
        with self.engine.begin() as connection:
            connection.execute(delete(records).where(records.c.record_type == self.record_type, records.c.record_id == record_id, records.c.business_id == business_id))
            connection.execute(insert(records).values(record_type=self.record_type, record_id=record_id, business_id=business_id, payload=payload, created_at=now, updated_at=now))
        return record

    def get(self, record_id: str, business_id: str | None = None) -> T | None:
        query = select(records.c.payload).where(records.c.record_type == self.record_type, records.c.record_id == record_id)
        if business_id is not None: query = query.where(records.c.business_id == business_id)
        with self.engine.connect() as connection: payloads = connection.execute(query).scalars().all()
        if not payloads: return None
        if len(payloads) > 1: raise LookupError("Record ID is ambiguous across businesses; supply business_id.")
        return self._decode(payloads[0])

    def list_for_business(self, business_id: str) -> list[T]:
        query = select(records.c.payload).where(records.c.record_type == self.record_type, records.c.business_id == business_id).order_by(records.c.created_at, records.c.record_id)
        with self.engine.connect() as connection: payloads = connection.execute(query).scalars().all()
